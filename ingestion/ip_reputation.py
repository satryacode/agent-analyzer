...
# Reasons that count as a hard/high-severity offense for reputation purposes.
...
class ReputationSummary:
    source_ip: str
    total_verdicts: int = 0          # verdicts seen for this IP in the window
    high_severity_verdicts: int = 0  # subset matching high-severity attack reasons
    already_blocked: bool = False    # IP already present in blocked_ips
    ...
    def summary_line(self) -> str:
        return (
            f"{self.total_verdicts} prior verdict(s) in window, "
            f"{self.high_severity_verdicts} high-severity, "
            f"{'already IP-blocked' if self.already_blocked else 'not yet blocked'}"
        )
...
class IpReputation:
    ...
    Both the classifier and the analyzer use this so the pipeline has memory:
    a single noisy request is judged in the context of the IP's recent history.
    """
